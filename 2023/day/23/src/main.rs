...
fn do_it_again(input: &str) -> usize {
    let (nodes, start, end) = parse_again(input);
...
fn parse_again(
    ...
    let all: Vec<_> = input
        ...
        .flat_map(|(y, l)| {
            l.chars().enumerate().filter_map(move |(x, c)| match c {
                '#' => None,
                _ if y > 0 => Some(((x, y), vec![(x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)])),
                _ => Some(((x, y), vec![(x - 1, y), (x + 1, y), (x, y + 1)])),
            })
        })
        .collect();
...
fn test() {
    ...
    assert_eq!(do_it_again(input.as_str()), 154);
}
